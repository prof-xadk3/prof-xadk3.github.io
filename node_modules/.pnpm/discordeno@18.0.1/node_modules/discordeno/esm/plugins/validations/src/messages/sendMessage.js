...
export function sendMessage(bot) {
    ...
    bot.helpers.sendMessage = function (channelId, content) {
        ...
        if (!content.content?.length && !content.embeds?.length && !content.components?.length && !content.file &&
            !content.stickerIds?.length) {
            throw new Error("When sending a message, you must provide a value for at least one of content, embeds, stickerIds, components, or file.");
        }
