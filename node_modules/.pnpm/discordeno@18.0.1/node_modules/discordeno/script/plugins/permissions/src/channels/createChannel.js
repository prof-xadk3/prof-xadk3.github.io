...
function createChannel(bot) {
    ...
    bot.helpers.createChannel = async function (guildId, options) {
        ...
        if (guild) {
            ...
            if (options?.type === deps_js_1.ChannelTypes.GuildAnnouncement && !guild.toggles.has("news")) {
                throw new Error("The NEWS feature is missing in this guild to be able to modify the channel type.");
            }
            if (options?.topic && !bot.utils.validateLength(options.topic, {
                min: 1,
                max: options.type === deps_js_1.ChannelTypes.GuildForum ? 4096 : 1024,
            })) {
            ...
            (0, permissions_js_1.requireBotGuildPermissions)(bot, guild, requiredPerms);
        }
