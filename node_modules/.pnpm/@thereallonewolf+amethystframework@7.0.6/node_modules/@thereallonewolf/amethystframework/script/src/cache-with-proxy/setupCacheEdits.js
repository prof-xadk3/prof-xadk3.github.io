...
exports.setupCacheEdits = exports.unavailablesGuilds = void 0;
exports.unavailablesGuilds = new Set();
function setupCacheEdits(bot) {
    const { GUILD_MEMBER_ADD, GUILD_MEMBER_REMOVE, MESSAGE_REACTION_ADD, MESSAGE_REACTION_REMOVE, MESSAGE_REACTION_REMOVE_ALL, CHANNEL_UPDATE, MESSAGE_UPDATE, GUILD_UPDATE, GUILD_ROLE_UPDATE, GUILD_MEMBER_UPDATE, USER_UPDATE, } = bot.handlers;
    ...
    bot.handlers.MESSAGE_REACTION_ADD = async function (_, data, shardId) {
        ...
        const message = await bot.cache.messages.get(messageId, bot.transformers.snowflake(payload.channel_id), undefined);
    ...
    bot.handlers.MESSAGE_REACTION_REMOVE_ALL = async function (_, data, shardId) {
        const payload = data.d;
        const messageId = bot.transformers.snowflake(payload.message_id);
        const message = await bot.cache.messages.get(messageId, bot.transformers.snowflake(payload.channel_id));
        if (message) {
    ...
    bot.handlers.MESSAGE_UPDATE = async function (_, data, shardId) {
        ...
        if (!payload.edited_timestamp)
            return;
        const oldMessage = await bot.cache.messages.get(BigInt(payload.id), bot.transformers.snowflake(payload.channel_id), undefined, false);
    ...
    bot.handlers.GUILD_UPDATE = function (_, data, shardId) {
        ...
        if (payload.unavailable)
            exports.unavailablesGuilds.add(guildID);
        // otherwise remove from Set
        else
            exports.unavailablesGuilds.delete(guildID);
        GUILD_UPDATE(bot, data, shardId);
    };
}
