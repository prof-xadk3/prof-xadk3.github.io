...
exports.AmethystCollection = void 0;
class AmethystCollection extends Map {
    /**
    ...
    reduce(fn, initialValue) {
        ...
        // No items iterated.
        if (first) {
            throw new TypeError("Reduce of empty AmethystCollection with no initial value");
        }
        return accumulator;
    }
    each(fn, thisArg) {
     ...
     */
    equals(AmethystCollection) {
        if (!AmethystCollection)
            return false; // runtime check
        if (this === AmethystCollection)
            return true;
        if (this.size !== AmethystCollection.size)
            return false;
        for (const [key, value] of this) {
            if (!AmethystCollection.has(key) ||
     ...
     * ```
     */
    sort(compareFunction = AmethystCollection.defaultSort) {
     ...
     * ```
     */
    sorted(compareFunction = AmethystCollection.defaultSort) {
    ...
    static combineEntries(entries, combine) {
        const coll = new AmethystCollection();
        for (const [k, v] of entries) {
            if (coll.has(k)) {
...
exports.AmethystCollection = AmethystCollection;
