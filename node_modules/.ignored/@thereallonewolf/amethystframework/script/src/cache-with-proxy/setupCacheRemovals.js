...
function setupCacheRemovals(bot) {
    ...
    bot.handlers.GUILD_DELETE = async function (_, data, shardId) {
        ...
        setupCacheEdits_js_1.unavailablesGuilds.delete(id);
        GUILD_DELETE(bot, data, shardId);
    };
    bot.handlers.CHANNEL_DELETE = async function (_, data, shardId) {
        const payload = data.d;
        const channel = await bot.cache.channels.get(BigInt(payload.id), undefined, false);
    ...
    bot.handlers.GUILD_MEMBER_REMOVE = async function (_, data, shardId) {
        ...
        const member = await bot.cache.members.get(BigInt(payload.user.id), BigInt(payload.guild_id), false);
    ...
    bot.handlers.MESSAGE_DELETE = async function (_, data) {
        const payload = data.d;
        const id = bot.transformers.snowflake(payload.id);
        const message = await bot.cache.messages.get(BigInt(payload.id), BigInt(payload.channel_id), payload.guild_id ? BigInt(payload.guild_id) : undefined, false);
