...
exports.createProxyCache = void 0;
const deps_js_1 = require("../../deps.js");
const setupCacheCreations_js_1 = require("./setupCacheCreations.js");
const setupCacheEdits_js_1 = require("./setupCacheEdits.js");
const setupCacheRemovals_js_1 = require("./setupCacheRemovals.js");
const updateHandlers = () => {
    const handlers = (0, deps_js_1.createBotGatewayHandlers)({});
    return {
        ...handlers,
        GUILD_LOADED_DD: (bot, data, shardId) => { },
    };
};
function createProxyCache(rawBot, options) {
    ...
    const fetchers = {
        fetchGuild: async (id, fetch, getMemory) => {
            ...
            if (setupCacheEdits_js_1.unavailablesGuilds.has(id))
                return;
        ...
        fetchRole: async (guildId, roleId, fetch) => {
            if (fetch === false)
                return;
            if (setupCacheEdits_js_1.unavailablesGuilds.has(guildId))
                return;
        ...
        fetchChannel: async (channelId, guildId, fetch) => {
            if (fetch === false)
                return;
            if (guildId && setupCacheEdits_js_1.unavailablesGuilds.has(guildId))
                return;
    ...
    bot.cache.guilds = {
        memory: new deps_js_1.Collection(),
    ...
    bot.cache.roles = {
        guildIDs: new deps_js_1.Collection(),
        memory: new deps_js_1.Collection(),
            ...
            if (options.cacheInMemory?.roles) {
                ...
                if (options.cacheInMemory?.guilds) {
                    ...
                    if (guildID) {
                        ...
                        if (guild) {
                            if (!guild.roles)
                                guild.roles = new deps_js_1.Collection();
            ...
            if (options.cacheInMemory?.channels) {
                ...
                if (options.cacheInMemory?.guilds) {
                    ...
                    if (guildID) {
                        ...
                        if (guild) {
                            if (!guild.channels)
                                guild.channels = new deps_js_1.Collection();
    ...
    bot.cache.messages = {
        channelIDs: new deps_js_1.Collection(),
        memory: new deps_js_1.Collection(),
            ...
            if (options.cacheInMemory?.messages) {
                if (options.cacheInMemory?.channels) {
                    if (message.channelId && message.guildId) {
                        ...
                        if (channel) {
                            if (!channel.messages)
                                channel.messages = new deps_js_1.Collection();
    ...
    bot.transformers.guild = function (_, payload) {
        if (options.cacheInMemory?.guilds) {
            ...
            const preCacheGuild = {
                toggles: new deps_js_1.GuildToggles(payload.guild),
                ...
                icon: payload.guild.icon
                    ...
                    : undefined,
                channels: new deps_js_1.Collection(),
                roles: new deps_js_1.Collection(),
                id: guildId,
        ...
        // Filter to desired args
        const args = {
            members: new deps_js_1.Collection(),
        };
        const keys = Object.keys(old);
        for (const key of keys) {
    ...
    bot.transformers.channel = function (_, payload) {
        ...
        old.messages = new deps_js_1.Collection();
        // Filter to desired args
        const args = {};
        const keys = Object.keys(old);
        for (const key of keys) {
    ...
    (0, setupCacheCreations_js_1.setupCacheCreations)(bot);
    return bot;
}
exports.createProxyCache = createProxyCache;
