...
exports.validateApplicationCommandLength = void 0;
function validateApplicationCommandLength(options) {
    let length = 0;
    ...
    if (options.descriptionLocalizations) {
        length += Math.max(options.description.length, ...Object.values(options.descriptionLocalizations).map((name) => name.length));
    }
    else {
        length += options.description.length;
    }
    if (options.options)
        length += validateApplicationCommandOptionLength(options.options);
    if (length > 4000) {
        throw new Error("Slash commands can have a maximum of 4000 characters for combined name, description, and value properties for each command, its options (including subcommands and groups), and choices. When localization fields are present, only the longest localization for each field (including the default value) is counted towards the size limit.");
    }
}
exports.validateApplicationCommandLength = validateApplicationCommandLength;
function validateApplicationCommandOptionLength(options) {
    let length = 0;
    for (const option of options) {
        ...
        if (option.descriptionLocalizations) {
            length += Math.max(option.description.length, ...Object.values(option.descriptionLocalizations).map((name) => name.length));
        }
        else {
        ...
        if (option.options)
            length += validateApplicationCommandOptionLength(option.options);
    }
    return length;
}
