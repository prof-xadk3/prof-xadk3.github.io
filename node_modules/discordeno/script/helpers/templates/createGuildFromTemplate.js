 ...
 */
async function createGuildFromTemplate(bot, templateCode, options) {
    if (options.icon) {
        options.icon = await bot.utils.urlToBase64(options.icon);
    }
    const createdGuild = await bot.rest.runMethod(bot.rest, "POST", bot.constants.routes.TEMPLATE(templateCode), options);
